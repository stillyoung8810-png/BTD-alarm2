-- ============================================
-- Benefit reward RPCs for Toss point Phase 2
-- ============================================
-- All money mutations run inside a single Postgres transaction.
-- Wallet, mission-state, and target rows are locked with FOR UPDATE
-- before ledger entries update balances.
-- ============================================
...
BEGIN
  ...
  IF v_base_ledger_id IS NOT NULL THEN
    ...
    SET
      money_balance = money_balance + 1,
      lifetime_earned_money = lifetime_earned_money + 1,
      updated_at = v_now
    WHERE user_id = p_user_id
  ...
  v_is_streak_bonus_day :=
    v_attendance.consecutive_days % 10 = 0;
  ...
  THEN
    ...
    VALUES (
      ...
      p_attendance_date::text,
      10,
      v_wallet.money_balance + 10
    )
    ON CONFLICT (user_id, source, source_id) DO NOTHING
    RETURNING id INTO v_streak_ledger_id;

    IF v_streak_ledger_id IS NOT NULL THEN
      v_streak_bonus_money := 10;

      UPDATE public.benefit_wallets
      SET
        money_balance = money_balance + 10,
        lifetime_earned_money = lifetime_earned_money + 10,
        updated_at = v_now
      WHERE user_id = p_user_id
      RETURNING * INTO v_wallet;
...
BEGIN
  ...
  IF v_state.rewarded_ad_unlocks >= 4 THEN
    RETURN jsonb_build_object(
      'canGrant', false,
...
BEGIN
  ...
  IF v_ledger_id IS NOT NULL THEN
    ...
    SET
      ...
      lifetime_earned_money = lifetime_earned_money + 5,
      updated_at = v_now
    WHERE user_id = p_user_id
    RETURNING * INTO v_wallet;
  END IF;
...
CREATE OR REPLACE FUNCTION public.lock_and_create_pending_toss_redeem(
  p_user_id uuid,
...
DECLARE
  v_now timestamptz := now();
  v_wallet public.benefit_wallets%ROWTYPE;
  v_existing_payout public.benefit_toss_point_payouts%ROWTYPE;
  v_payout public.benefit_toss_point_payouts%ROWTYPE;
  v_redeem_bundle_count integer := 0;
  v_redeemed_money integer := 0;
  v_toss_point_amount integer := 0;
  v_ledger_id uuid;
BEGIN
  IF p_user_id IS NULL THEN
    RAISE EXCEPTION 'user_id_required';
  ...
  IF btrim(COALESCE(p_promotion_code, '')) = '' THEN
    RAISE EXCEPTION 'promotion_code_required';
  END IF;
  ...
  IF FOUND THEN
    RETURN jsonb_build_object(
      'alreadyProcessed', true,
      'payoutId', v_existing_payout.id,
      'status', v_existing_payout.status,
      'redeemedMoney', v_existing_payout.redeemed_money,
      'tossPointAmount', v_existing_payout.toss_point_amount,
      'moneyBalance', v_wallet.money_balance
    );
  END IF;

  v_redeem_bundle_count := LEAST(
    v_wallet.money_balance / 1000,
    50
  );

  IF v_redeem_bundle_count <= 0 THEN
    RAISE EXCEPTION 'not_enough_money_to_redeem';
  END IF;

  v_redeemed_money := v_redeem_bundle_count * 1000;
  v_toss_point_amount := v_redeem_bundle_count * 100;

  UPDATE public.benefit_wallets
  SET
    money_balance = money_balance - v_redeemed_money,
    updated_at = v_now
  WHERE user_id = p_user_id
  RETURNING * INTO v_wallet;
  ...
  VALUES (
    ...
    p_redeem_request_id,
    -v_redeemed_money,
    v_wallet.money_balance
  )
  ON CONFLICT (user_id, source, source_id) DO NOTHING
  RETURNING id INTO v_ledger_id;

  IF v_ledger_id IS NULL THEN
    RAISE EXCEPTION 'redeem_debit_ledger_already_exists';
  END IF;

  INSERT INTO public.benefit_toss_point_payouts (
    user_id,
    redeem_request_id,
    promotion_code,
    redeemed_money,
  ...
  RETURN jsonb_build_object(
    ...
    'redeemedMoney', v_redeemed_money,
    'tossPointAmount', v_toss_point_amount,
    'moneyBalance', v_wallet.money_balance
  );
END;
$$;
...
REVOKE EXECUTE ON FUNCTION public.unlock_benefit_mission_ad(uuid, text, date) FROM PUBLIC, anon, authenticated;
...
REVOKE EXECUTE ON FUNCTION public.lock_and_create_pending_toss_redeem(uuid, text, text) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.attend_and_claim_reward(uuid, date, boolean) TO service_role;
GRANT EXECUTE ON FUNCTION public.unlock_benefit_mission_ad(uuid, text, date) TO service_role;
...
GRANT EXECUTE ON FUNCTION public.lock_and_create_pending_toss_redeem(uuid, text, text) TO service_role;
