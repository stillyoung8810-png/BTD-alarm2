...
**상태**: 설계 전용 — **프로덕션 코드 변경 없음** (승인 후 구현)
...
**범위**: `INTERSTITIAL_SETTLEMENT_DETAIL` 한 지면의 트리거 위치만 조정한다. `adService.ts`의 load/show 파이프라인·티어 가드는 변경하지 않는 것을 권장한다.
...
| 9 | 광고 로딩 중 재호출·쿨타임만으로 부족 | **동의(§0.5).** `isAdPipelineActiveRef`로 **`showInterstitialOnTransition` 동시 실행** 방지. |
...
2. **`lastSettlementExitInterstitialShownAtMsRef` + `SETTLEMENT_DETAIL_EXIT_INTERSTITIAL_COOLDOWN_MS` (제품 정책, §0.2)**  
   ...
   - 검사는 **`showInterstitialOnTransition` 호출 직전**에 하며, `setDetailsTargetId(null)` **이후**에 실행되므로 UI 닫힘은 항상 보장된다.
...
| **시작 시점** | 정산 상세 모달을 닫는 흐름에서 **`showInterstitialOnTransition`이 반환한 `AdResult`의 `shown === true`**인 경우에만, 그 시각을 `lastSettlementExitInterstitialShownAtMsRef`에 기록한다. |
...
**금지 (여전히 유효)**: UI 닫기 가능성을 막기 위해 **`showInterstitialOnTransition`의 `await` 완료**까지 UI 전용 락을 잡아 두는 것(§0.3 v1.3 데드락 논의와 동일).

**허용 (광고 파이프라인 전용)**: `isAdPipelineActiveRef`는 **`showInterstitialOnTransition` 비동기 IIFE의 `finally`에서만** 해제한다 — UI 닫기와 생명주기가 분리된다(§0.5).

### 0.5 광고 파이프라인 동시성 락 (필수)

**문제**: A 모달 닫기로 광고 **로딩 중**(아직 `result.shown` 전)에 B 모달을 닫으면, **5초 쿨타임**은 첫 노출 시각이 없어 통과할 수 있고 **`showInterstitialOnTransition`이 중복 호출**될 수 있다.
...
sequenceDiagram
  ...
  participant Ad as showInterstitialOnTransition
...
| `App`의 `PortfolioDetailsModal` `onClose` | 항상 `setDetailsTargetId(null)`만 | **종료된 포트폴리오(`isClosed === true`)**일 때만 종료 후 `showInterstitialOnTransition` 호출 |
...
- `services/ads/adService.ts` — `showInterstitialOnTransition`, `AdPlacement.INTERSTITIAL_SETTLEMENT_DETAIL`
...
// App 컴포넌트 내부:
/** `showInterstitialOnTransition` 비동기 호출이 겹치지 않도록 */
...
```typescript
import type { UserTier } from './services/ads/adService';
```
...
const handlePortfolioDetailsModalClose = useCallback(() => {
  ...
  const tierForAd: UserTier =
    currentTier === 'pro' || currentTier === 'premium' ? currentTier : 'free';
...
}, [detailsTargetId, currentTier, setDetailsTargetId]);
```
...
**Why `isSettlementExitAdPipelineActiveRef` (§0.5)**: 첫 노출 전 로딩 구간에서 **두 번째 `showInterstitialOnTransition` 호출**이 나가지 않는다. 해제는 **광고 IIFE의 `finally`만** — UI와 분리.
...
| **전면 광고 노출 직후** 짧은 시간 안에 다른 상세 열고 닫기 | **5초 쿨타임**으로 `showInterstitialOnTransition` 호출 생략; `setDetailsTargetId(null)`은 항상 실행 |
| **쿨타임 중** 다른 정산 상세 열고 닫기 | 모달은 정상 동작; 전면만 스킵 |
| Dashboard 상세 닫기 | `isClosed === false` → 광고 분기 미진입 |
| `showInterstitialOnTransition`이 `shown: false`만 반환 | 쿨타임 타임스탬프 **미갱신** → 다음 닫기에서 다시 시도 가능 |
| `showInterstitialOnTransition` throw | `catch` 로깅; `finally`에서 파이프라인 락 해제; 모달은 이미 닫힘 |
