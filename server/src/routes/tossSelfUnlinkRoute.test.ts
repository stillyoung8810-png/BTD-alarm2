...
function mockStoredLinkRecord(options: {
  byAuthUserId: {
  ...
  profileTossUserKey?: string | null;
}): void {
          ...
          eq: (field: string, value: string) => ({
            maybeSingle: async () => {
              if (columns !== 'toss_user_key, encrypted_refresh_token') {
                throw new Error(`Unexpected toss_auth_links columns: ${columns}`);
              }

              if (field === 'auth_user_id') {
                return {
                  data: options.byAuthUserId,
                  error: null,
                };
              }
              ...
              if (field === 'toss_user_key') {
                return {
                  data: options.byTossUserKey ?? null,
                  error: null,
                };
              }
    ...
    if (table === 'user_profiles') {
      return {
        select: (columns: string) => ({
          eq: (field: string, value: string) => ({
            maybeSingle: async () => {
              if (columns !== 'toss_user_key' || field !== 'id') {
                throw new Error(`Unexpected user_profiles query: ${columns} ${field} ${value}`);
              }

              return {
                data:
                  options.profileTossUserKey == null
                    ? null
                    : { toss_user_key: options.profileTossUserKey },
                error: null,
              };
            },
          }),
...
describe('tossSelfUnlinkRoute', () => {
  ...
  it('auth_user_id 조회가 비어도 user_profiles.toss_user_key fallback으로 unlink를 계속한다', async () => {
    mockStoredLinkRecord({
      byAuthUserId: null,
      byTossUserKey: {
        toss_user_key: '123',
