...
```tsx
import React, { useCallback, useId, type KeyboardEvent, type ReactNode } from 'react';
...
export const TdsDialogShell: React.FC<TdsDialogShellProps> = ({
  ...
  if (isInTossApp) {
    return (
      <TDSModal open={isOpen} onClose={handleClose} onExited={onExited}>
        {dialogBody}
      </TDSModal>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      <div
        ...
        onClick={handleClose}
        onKeyDown={(event) => handleClosableKeyDown(event, handleClose)}
...
- Alert는 보통 단일 동의/확인 흐름이므로 `onConfirm` 후 `onClose`를 실행합니다.
- 별도 state mutation은 렌더 단계가 아닌 `handleConfirmClick`에서만 일어납니다.
...
- 닫힌 상태로 먼저 리셋한 뒤 실제 콜백을 실행하므로, 중첩 모달과 stale UI를 줄일 수 있습니다.
- 동일 패턴을 `AlertDialogState`에도 재사용할 수 있습니다.
  ...
  return (
    <>
      <AuthModals
        lang={lang}
        type="login"
        onClose={() => handleRequestExit('auth_close')}
        onSwitchType={() => {}}
        onLogin={() => {}}
        onLogout={() => {}}
      />
...
- `onClearHistory` 자체는 기존 비즈니스 로직을 그대로 사용합니다.
...
export const RefundGuideController: React.FC<RefundGuideControllerProps> = ({
  ...
  }, [dialogMessages.refund.guideBody, dialogMessages.refund.guideTitle, isInTossApp]);

  const handleConfirmRefundGuide = useCallback(() => {
    handleCloseAlertDialog();
    onCloseRefundPanel();
  }, [handleCloseAlertDialog, onCloseRefundPanel]);
...
- 토스 환경일 때만 안내 다이얼로그를 엽니다.
- 문구는 모두 사전에서 주입합니다.
- 환불 패널 닫기와 안내 다이얼로그 닫기를 **명시적 handler**로 분리해 side effect를 예측 가능하게 만듭니다.
