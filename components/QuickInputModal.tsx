...
const QuickInputModal: React.FC<QuickInputModalProps> = ({ lang, portfolio, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#161d2a] rounded-[2.5rem] border border-white/10 shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="p-8 border-b border-white/5 flex justify-between items-center">
          <div className="flex items-center gap-3">
             ...
             <h2 className="text-xl font-black text-white tracking-tight">{t.quickInput}</h2>
          </div>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="flex p-1.5 bg-slate-900 rounded-2xl border border-white/5">
            <button onClick={() => setType('buy')} className={`flex-1 py-4 rounded-xl text-xs font-black transition-all ${type === 'buy' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}>{t.buy}</button>
            <button onClick={() => setType('sell')} className={`flex-1 py-4 rounded-xl text-xs font-black transition-all ${type === 'sell' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500'}`}>{t.sell}</button>
          </div>
              ...
              <div className="flex gap-2">
                {[1, 2, 3].map(sec => (
                  <button 
                    ...
                    className={`flex-1 py-3 rounded-xl text-xs font-black border transition-all ${activeSection === sec ? 'bg-blue-600 border-blue-500 text-white shadow-lg' : 'bg-slate-900 border-white/5 text-slate-500'}`}
                  >
                    {t.section} {sec}
                  </button>
                ))}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="0.00"
                  className="w-full p-5 pl-10 bg-slate-900 border border-white/5 rounded-2xl text-white font-black text-lg focus:ring-2 focus:ring-blue-500/50 outline-none" 
                />
              </div>
            </div>
            ...
            {type === 'sell' && (
              <div className="space-y-2 animate-in slide-in-from-top-2">
                ...
                <input 
                  ...
                  placeholder="0"
                  className="w-full p-5 bg-slate-900 border border-white/5 rounded-2xl text-white font-black text-lg focus:ring-2 focus:ring-blue-500/50 outline-none" 
                />
              </div>
            )}
          </div>
          ...
          <div className="bg-slate-900/50 p-6 rounded-[2rem] border border-white/5 space-y-4">
             <div className="flex justify-between items-center text-slate-400">
               ...
               <span className="text-lg font-black text-white">{quantity || '0'}</span>
             </div>
             <div className="flex justify-between items-center text-slate-400">
               <span className="text-[11px] font-bold uppercase tracking-widest">{t.calculatedFee}:</span>
               <span className="text-sm font-black text-white">${commission.toFixed(2)}</span>
             </div>
             {type === 'sell' && (
               <div className="flex justify-between items-center text-slate-400 animate-in fade-in duration-300">
                 <span className="text-[11px] font-bold uppercase tracking-widest">{t.secFee}</span>
                 <span className="text-sm font-black text-white">${secFee.toFixed(4)}</span>
               </div>
             )}
             <div className="h-[1px] bg-white/5"></div>
             <div className="flex justify-between items-center">
               <span className="text-[11px] font-black text-white uppercase tracking-widest">{t.totalAmount}:</span>
               <span className="text-xl font-black text-blue-500">${totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
             </div>
          </div>
        </div>

        <div className="p-8 border-t border-white/5 flex gap-4 bg-slate-900/30">
          <button onClick={onClose} className="flex-1 py-5 bg-slate-800 text-slate-400 rounded-2xl font-black uppercase text-xs hover:bg-slate-700 transition-all">{t.cancel}</button>
          <button onClick={handleSave} className="flex-[2] py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs shadow-xl shadow-blue-500/20 flex items-center justify-center gap-2 hover:scale-[1.02] transition-all">
