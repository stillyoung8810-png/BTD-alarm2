...
실제 유사 출시 사례와 토스 측 확인 내용을 기준으로, 이번 기능은 아래 전제를 만족하는 범위에서만 운영합니다.
...
| `5,000P` 한도 해석 | 현재 운영 전제는 `1회 지급 한도 5,000P`입니다. 동일 유저가 기간 중 여러 번 받을 수 있더라도 각 요청은 `5,000P`를 넘지 않도록 서버가 강제합니다. |
| 최종 기준 | 공개 문서와 별도로 토스 측 답변을 받은 항목은 해당 확인 내용을 운영 메모로 보관하고, 검수 재확인 요청 시 즉시 제출할 수 있게 관리합니다. |

검수 제출 전 고객-facing 표현은 아래처럼 정리합니다.
...
토스 포인트 프로모션은 금액별로 여러 개 만들지 않고, 승인된 단일 `promotionCode`를 사용합니다. 실제 지급 포인트는 API 호출 시 `amount`로 전달하며, 이 값은 내부 `머니`가 아니라 토스 포인트 수량입니다. 예를 들어 `1,000머니 -> 100P` 요청이면 `amount = 100`, `50,000머니 -> 5,000P` 요청이면 `amount = 5000`입니다. 정책 확인 결과 `5,000P`는 **1회 요청 한도**이므로, 같은 유저가 기간 중 여러 번 받더라도 각 요청은 `5,000P`를 넘지 않게 서버가 강제합니다.

고객 안내 문구와 검수용 표현은 아래 기준을 따릅니다.
...
2. 프로모션 예산, 요청별 최대 `5,000P`, 일일 운영 한도 결정
3. 프로모션 S2S 운영 사전 체크: 계약/정산 승인, mTLS/파트너 인증, 테스트 계정 `x-toss-user-key`, `Content-Type: application/json` 요청 설정 확인
4. 승인 완료 후 `TEST_{promotionCode}`로 최소 1회 포인트 지급 API 테스트 완료
5. 검수용 문구에 지급 조건, 지급 시점, 지급 제한, `본 프로모션은 사전 고지 없이 중단될 수 있습니다` 포함
6. 광고 그룹 ID 확인: 1차는 기존 배너/전면/보상형 ID 재사용, 콘솔·검수 문구가 혜택 미션과 맞지 않으면 혜택 전용 ID 발급 후 주입
7. `/benefits` 직접 진입 링크 정책 확정: `ROUTE_PATHS`, `syncTabFromLocation`, 탭 클릭 시 URL 동기화 방식 합의
8. `docs2/toss_point/quiz_seed/*.jsonl` 600문항 내용 검수, 오답/표현 리스크 제거, 승인 문항 `approved` 확정
9. JSONL → `benefit_quiz_questions` import 스크립트 또는 SQL seed 준비, `human_id` 기준 upsert 검증
10. 시뮬레이션 통과
11. 서버 원장/중복 지급/광고 해금 동시성 테스트 작성
12. 상단 월렛 보드 UI 구현
13. 프론트 UI 구현 시작
