...
export const loadInitialStockData = async (): Promise<void> => {
  try {
    ...
    if (DEBUG_STOCK_LOG) {
      console.log(
        "[loadInitialStockData] 초기 데이터 로딩 시작:",
        todayKst,
        "(UTC:",
        todayUtc,
        ")",
    ...
    await Promise.all(
      AVAILABLE_STOCKS.map(async (symbol) => {
        try {
          ...
          if (error || !data || data.length === 0) {
            console.warn(
              `[loadInitialStockData] ${symbol}: 데이터 없음`,
              error,
            );
            return;
          }
          ...
          if (DEBUG_STOCK_LOG) {
            console.log(
              `[loadInitialStockData] ${symbol}: ${records.length}일치 데이터 저장 완료`,
            );
          }
        } catch (err) {
...
export const loadPaidStockData = async (): Promise<void> => {
  try {
    ...
    if (DEBUG_STOCK_LOG) {
      console.log(
        "[loadPaidStockData] 유료 종목 데이터 로딩 시작:",
        todayKst,
        "(UTC:",
        todayUtc,
        ")",
    ...
    await Promise.all(
      PAID_STOCKS.map(async (symbol) => {
        try {
          ...
          if (DEBUG_STOCK_LOG) {
            console.log(
              `[loadPaidStockData] ${symbol}: ${records.length}일치 데이터 저장 완료`,
            );
          }
        } catch (err) {
...
export const calculateTechnicalIndicators = async (
  ...
  try {
    ...
    const prices = data.map((row) => (row as any).close).filter((p) =>
      p != null && p > 0
    ) as number[];
