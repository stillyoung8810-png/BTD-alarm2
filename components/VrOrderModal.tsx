...
const TABLE_COLUMNS: Array<{
...
}> = [
  {
    ...
    renderCell: (order, t) => {
      if (order.step === STEP_CURRENT_STATE) {
        return (
          <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
            {t.currentState}
          </span>
        );
      }
  ...
  { id: 'poolAfter', labelKey: 'poolAfter', align: 'right', format: 'decimal' },
];
        ...
        <thead>
          <tr className="border-b border-slate-200 dark:border-white/10">
            {TABLE_COLUMNS.map((col) => (
              <th
                key={col.id}
                className={`py-2 px-2 text-slate-500 dark:text-slate-400 font-semibold ${col.align === 'left' ? 'text-left' : 'text-right'} ${col.headerClass ?? ''}`}
              >
                {labels[col.labelKey]}
              </th>
            ))}
...
export default function VrOrderModal({
  ...
  return (
    <div className="fixed inset-0 z-[220] flex items-center justify-center p-4">
      ...
      />
      <div className="relative w-full max-w-md bg-white dark:bg-[#161d2a] rounded-[2rem] border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900/40">
